    ...
    return (
        <section id="contact" className="relative section-padding bg-gradient-to-br from-gray-50 via-white to-blue-50 overflow-hidden">
            ...
            <div className="container-apple relative z-10">
                <div ref={staggerRef} className="space-y-16">
                    {/* Header */}
                    <motion.div
                    ...
                    <div className="grid lg:grid-cols-5 gap-12 lg:gap-16">
                        ...
                        >
                            ...
                            <div className="space-y-4">
                                {contactInfo.map((info, index) => (
                                    <motion.a
                                        ...
                                        className="group relative flex items-center gap-4 p-5 bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 transition-all duration-300 overflow-hidden"
                                        initial={{ opacity: 0, y: 20 }}
                                        whileInView={{ opacity: 1, y: 0 }}
                                        viewport={{ once: true }}
                                        ...
                                        whileHover={{ x: 8 }}
                                    >
                                        <div className={`relative w-14 h-14 bg-gradient-to-br ${info.color} rounded-xl flex items-center justify-center shadow-lg`}>
                                            <info.icon size={24} className="text-white relative z-10" />
                                            <div className="absolute inset-0 bg-white/20 rounded-xl group-hover:scale-110 transition-transform" />
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="font-semibold text-gray-900 mb-1">{info.title}</h4>
                                            <p className="text-gray-600 text-sm">{info.value}</p>
                                        </div>
                                        <ArrowUpRight size={20} className="text-gray-400 group-hover:text-blue-600 group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                                    </motion.a>
                                ))}
                            </div>
                            ...
                            <motion.div
                                ...
                                viewport={{ once: true }}
                            >
                                <h4 className="text-lg font-semibold text-gray-900 mb-5">{t.contact.followMe}</h4>
                                <div className="flex gap-3">
                                    {socialLinks.map((social, index) => (
                                        <motion.a
                                            ...
                                            className={`group relative w-14 h-14 bg-gray-100 rounded-xl flex items-center justify-center text-gray-700 ${social.color} hover:text-white transition-all duration-300 shadow-sm hover:shadow-lg overflow-hidden`}
                        ...
                        <motion.div
                            ...
                            viewport={{ once: true }}
                        >
                            <div className="relative bg-gradient-to-br from-white via-white to-blue-50/30 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-200/60 p-8 md:p-12 overflow-hidden">
                                {/* 배경 패턴 */}
                                ...
                                <form onSubmit={handleSubmit} className="relative space-y-7" aria-describedby={error ? 'contact-error' : undefined}>
                                    {/* Success/Error Messages */}
                                    {isSubmitted && (
                                        <motion.div
                                            className="p-5 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl flex items-center gap-3"
                                            ...
                                            <div className="relative flex items-center justify-center gap-3">
                                                {isSubmitting ? (
                                                    <>
                                                        ...
                                                        <span className="font-semibold">{t.contact.form.sending}</span>
                                                    </>
                                                ) : isSubmitted ? (
                                                    <>
                                                        <CheckCircle size={24} className="animate-bounce" />
                                                        <span className="font-semibold">{t.contact.form.messageSent}</span>
                                                    </>
                                                ) : (
                                                    <>
                                                        <Send size={24} className="group-hover:rotate-12 transition-transform" />
                                                        <span className="font-semibold tracking-wide">{t.contact.form.sendMessage}</span>
                                                        <ArrowUpRight size={22} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300" />
                                                    </>
                                                )}
                                            </div>
